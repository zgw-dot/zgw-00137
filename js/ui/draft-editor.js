    ...
    class Editor {
        ...
        _showPublishConflict(config, type, existingData) {
            ...
            if (type === 'builtin') {
                ...
                const existingJson = existingLevelData;
                const newJson = config;
                ...
                fields.forEach(f => {
                    ...
                    row.innerHTML = `<td>${f.label}</td><td class="${changed ? 'changed' : ''}">${oldVal}</td><td class="${changed ? 'changed' : ''}">${newVal}</td>`;
                    diffBody.appendChild(row);
                });

                const oldCount = (existingJson.orders || []).length;
                const newCount = (newJson.orders || []).length;
